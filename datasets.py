    ...
    def __next__(self):
        ...
        if self.pipe == 0:  # local camera
            ...
            while True:
                n += 1
                self.cap.grab()
                if n % 4 == 0:  # skip frames  ktra rút gọn khung hình,1/4 thì ktra 1
                    ret_val, img0 = self.cap.retrieve()
                    # ret_val ảnh trả về đúng thì T, không lấy được thì F
                    if not ret_val:
                        # print("aaaaaaaaaaaa")
                        print("mất kết nối")
                        self.cap.release()  # nếu mất nguồn thì tắt cam đang lỗi đi
                        self.cap = cv2.VideoCapture(self.pipe)  # kdong lại
                        ret_val, img0 = self.cap.retrieve()  # thử lấy ảnh coi
                        print("Kết nối lại")
                    else:
                        break
